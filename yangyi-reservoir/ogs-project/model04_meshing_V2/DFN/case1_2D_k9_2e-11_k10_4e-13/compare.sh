#!/usr/bin/env bash
set -euo pipefail
python extract_monitoring_head_change_2d.py --results output/case1_k9_2e-11_k10_4e-13_200d.pvd --monitoring monitoring/monitoring_nodes_2d_fractures_filtered.csv --outdir head_change_output
python compare_simulated_observed_head_change_after20190113.py --sim-heads head_change_output/simulated_head_timeseries.csv --obs-xlsx monitoring/yangyi_after_20190113_for_ogs.xlsx --outdir comparison_output
